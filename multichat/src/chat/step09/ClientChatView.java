...
public class ClientChatView extends JFrame {
	...
	public ClientChatView(String ip, int port, String nickname) {
		...
		scroll.setBounds(12, 50, 472, 320);
				...
				public void run() {
					while(true) {
						...
						try {
							...
							//e.printStackTrace(); - //개발전용
							try {
								//=====2. 서버쪽에서 연결에서 끊어지는 경우 사용자는 자원을 반납=====
								//자원반납하기
								is.close();
								ir.close();
								br.close();
	...
	private void filteringMsg(String msg) {
		...
		//====1. 서버가 전달한 메시지의 프로토콜이 out이면 bnickname리스트에서 해당 nickname을 제거====	
